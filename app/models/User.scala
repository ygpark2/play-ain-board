...
class Users @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends GenericCrud[User] {
  ...
  */
  ...
  protected class UsersTable(tag: Tag) extends AbstractTable(tag, TableName) {
    ...
    def * = (id, common, name, email, emailConfirmed, password, salt, level, point, userInfo, userConf, role) <> (User.tupled, User.unapply)
  ...
  override protected val testData = {
    ...
    List(
      User(UUID.randomUUID(), Common(), "test_name1", "email1@abc.com", false, "password".bcrypt(salt), salt, 2, 283, userInfo, userConf, NormalRole),
      User(UUID.randomUUID(), Common(), "test_name2", "email2@abc.com", false, "password".bcrypt(salt), salt, 4, 383, userInfo, userConf, NormalRole)
    )
  }
}
