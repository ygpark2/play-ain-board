...
class Post @Inject()(val config: Config,
  ...
  def newPost(board_key: String) = Action { request =>
    val board = Await.result( boards.findByKey(board_key), Duration.Inf)
    board match {
      case Some(b) => {
        Ok(views.html.board.post.newForm(postForms.newForm, b))
      }
      case _ => {
  ...
  def handleNewPost(board_key: String) = Action.async { implicit request =>
    println("-----------------------------------------")
    ...
    postForms.newForm.bindFromRequest.fold(
      formWithErrors => Future.successful(BadRequest(views.html.board.post.newForm(formWithErrors, boardVal))),
      postFormData => {
        val salt = generateSalt
        println("--------------- password matched -------------------------------")
        val postUser = PostUser("userid", "username", "nickname", "email", "homepage")

        val postConf = PostConf(false, false, false, false, false)
        ...
        for {
          ...
          if (id > 0) {
            ...
            Ok(views.html.board.post.newForm(postForms.newForm, boardVal))
          }
        }
      }
    )
  ...
  def editPost(board_key: String, id: String) = Action { request =>
    ...
    board.head match {
      case (b, p) => {
        ...
        Ok(views.html.board.post.editForm(filledNewForm, b, p))
      }
      case _ => {
        Ok(views.html.error.notFound(request))
      }
  ...
  def handleEditPost(board_key: String, id: String) = Action.async { implicit request =>
    val boardList = Await.result(boards.findByIdJoinPost(id), Duration.Inf)
    if (boardList.length > 0) {
      val (board, post) = boardList.head

      postForms.newForm.bindFromRequest.fold(
        formWithErrors => Future.successful(BadRequest(views.html.board.post.newForm(formWithErrors, board))),
        postFormData => {
          ...
          for {
            ...
            if (id > 0) {
              ...
              Ok(views.html.board.post.newForm(postForms.newForm, board))
            }
          }
        }
      )
  ...
  def deletePost(board_key: String, id: String) = Action { request =>
    val board = Await.result(boards.findByIdJoinPost(id), Duration.Inf)
    board.head match {
      case (b, p) => {
  ...
  def viewPost(board_key: String, id: String) = Action { request =>
    val board = Await.result(boards.findByIdJoinPost(id), Duration.Inf)
    board.head match {
      case (b, p) => {
        // increase hit count
        ...
        Ok(views.html.board.post.view(p, b))
      }
      case _ => {
        Ok(views.html.error.notFound(request))
      }
  ...
  def listPost(board_key: String, page: Int, orderBy: Int, filter: String) = Action.async { implicit request =>
    val boardList = boards.postList(board_key, page = page, orderBy = orderBy, filter = ("%" + filter + "%"))
    boardList.map(ps => Ok(views.html.board.post.list(ps, orderBy, filter)))
  }
