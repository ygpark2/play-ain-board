...
import config.{AinAuthenticator, AinHttpActionAdapter, CustomAuthorizer, UsersProfileCreator}
import models.Users
...
class SecurityModule(environment: Environment, configuration: Configuration) extends AbstractModule {
  ...
  // @Inject val ainAuthenticator: AinAuthenticator
  // @Inject() var ainAuthenticator: AinAuthenticator
  // @Inject() private var as: AinAuthenticator = _
  // lazy val ainAuthenticator: AinAuthenticator = as
  ...
  override def configure(): Unit = {
    ...
    final FormClient formClient = new FormClient("http://localhost:8080/loginForm", authenticator);
     */
    ...
    val formClient = new FormClient(baseUrl + "/login", new AinAuthenticator())
    ...
    */
    ...
    val clients = new Clients(baseUrl + "/callback", facebookClient, twitterClient, formClient,
      indirectBasicAuthClient, parameterClient, directBasicAuthClient, new AnonymousClient())

    val config = new Config(clients)
    config.addAuthorizer("admin", new RequireAnyRoleAuthorizer[Nothing]("ROLE_ADMIN"))
    config.addAuthorizer("custom", new CustomAuthorizer)
    config.setHttpActionAdapter(new AinHttpActionAdapter())
